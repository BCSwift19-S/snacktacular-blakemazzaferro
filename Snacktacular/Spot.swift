...
//  Snacktacular
//
//  Created by Blake Mazzaferro on 3/31/19.
...
class Spot: NSObject, MKAnnotation {
    ...
    func saveData(completed: @escaping (Bool) -> ()) {
        ...
        guard let postingUserID = (Auth.auth().currentUser?.uid) else {
            print("ERROR: couldn't save data because we didn't have valid posting userID")
    ...
    func updateAverageRating(completed: @escaping () -> ()) {
        ...
        reviewsRef.getDocuments { (querySnapshot, error) in
            guard error == nil else {
                print("Error: Failed to get query snapshot of reviews for reviews ref")
                return completed()
            }
            var ratingTotal = 0.0
            ...
            spotRef.setData(dataToSave) { error in
                guard error == nil else {
                    print("Error updating document in spot after getting average rating")
                    return completed()
                }
                completed()
            }
